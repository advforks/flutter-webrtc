...
- All desktop prebuilts come from one archive: [`libwebrtc.zip`](https://github.com/flutter-webrtc/flutter-webrtc/releases/download/v1.4.0/libwebrtc.zip) (same URL as Linux/Windows x64). Windows ARM64 uses `lib/windows-arm64/` inside that zip once published; see [third_party/BUILD_WINDOWS_ARM64.md](third_party/BUILD_WINDOWS_ARM64.md) to build and add them.
- Windows x64 uses `lib/win64/` from the same zip.
